...
app.post('/api/equipments', async (req, res) => {
    try {
        const { name, location, pivo_id, description, lat, lng } = req.body;
        if (!name || !pivo_id) return res.status(400).json({ error: 'name e pivo_id são obrigatórios' });
        const list = await readData();
        const item = {
            id:           Date.now().toString(),
            name:         name.trim(),
            location:     (location || '').trim(),
            pivo_id:      pivo_id.trim(),
            description:  (description || '').trim(),
            lat:          lat != null ? parseFloat(lat) || null : null,
            lng:          lng != null ? parseFloat(lng) || null : null,
            // Tópicos MQTT — espelham a estrutura do painel de pivô existente
            topic_command: `ags/irrigation/${pivo_id.trim()}/command`,
            topic_status:  `ags/irrigation/${pivo_id.trim()}/status`,
            topic_online:  `ags/irrigation/${pivo_id.trim()}/online`,
            topic_config:  `ags/irrigation/${pivo_id.trim()}/config`,
...
app.put('/api/equipments/:id', async (req, res) => {
    try {
        ...
        const { name, location, pivo_id, description, lat, lng } = req.body;
        const pid = (pivo_id || list[idx].pivo_id).trim();
        list[idx] = {
            ...list[idx],
            ...
            topic_command: `ags/irrigation/${pid}/command`,
            topic_status:  `ags/irrigation/${pid}/status`,
            topic_online:  `ags/irrigation/${pid}/online`,
            topic_config:  `ags/irrigation/${pid}/config`,
            updatedAt:     new Date().toISOString()
        };
        await saveData(list);
        res.json(list[idx]);
...
async function start() {
    ...
    const list = await readData();
    console.log(`GitHub OK — ${list.length} pivô(s).`);
